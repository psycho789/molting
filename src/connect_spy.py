...
class SpyAgent:
    def __init__(self, config_path=None):
        # Default to config.json in project root (one level up from src/)
        ...
        # Track active members per room (for RNG kick threats)
        self.active_members = defaultdict(set)  # room -> set of member names
        self.member_last_activity = defaultdict(dict)  # room -> {member: timestamp}
        
        # Seed random number generator properly (time + process ID for better randomness)
    ...
    def log_message(self, room, from_user, text, timestamp=None, is_response=False):
        """Log a message to the room's JSONL log file"""
        ...
        # Add is_response flag for response messages
        if is_response:
    ...
    def register(self):
        ...
        try:
            ...
            if response.status_code in [200, 201]:
                ...
                # Server-side errors (Bad Gateway, Service Unavailable, Gateway Timeout)
                error_name = {
                    502: "Bad Gateway",
                    503: "Service Unavailable", 
                    504: "Gateway Timeout"
                }.get(response.status_code, f"HTTP {response.status_code}")
                print(f"✗ Registration failed: {error_name} (HTTP {response.status_code})", flush=True)
                print(f"⚠ Server-side error: The nohumans.chat service appears to be down or unreachable.", flush=True)
                print(f"   This is not a client issue. The service will automatically retry connection.", flush=True)
                print(f"   Response preview: {response.text[:200]}...", flush=True)
                return False
            else:
                print(f"✗ Registration failed: HTTP {response.status_code}", flush=True)
                # Only print full response if it's not too long (avoid HTML error pages)
                response_text = response.text
                if len(response_text) > 500:
                    print(f"Response (truncated): {response_text[:500]}...", flush=True)
                else:
    ...
    def load_personality(self):
        ...
        try:
            # Path relative to project root (one level up from src/)
            personality_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), "personalities", "the-shining-ribbons.md")
    ...
    def process_response(self, text):
        ...
        # Strip agent name prefix if present (e.g., "the shining ribbons-abc123: message" -> "message")
        agent_name_lower = self.agent_name.lower()
        text_lower = text.lower()
        
        # Check for various prefix patterns
        patterns_to_remove = [
            rf"^{re.escape(agent_name_lower)}\s*:\s*",  # "the shining ribbons: "
            rf"^{re.escape(agent_name_lower)}\s+",  # "the shining ribbons "
            rf"^{re.escape(agent_name_lower)}-\w+\s*:\s*",  # "the shining ribbons-abc123: "
            rf"^{re.escape(agent_name_lower)}-\w+\s+",  # "the shining ribbons-abc123 "
        ]
        ...
        # Check if text starts with agent name (with optional suffix like "-abc123")
        if text_lower.startswith(agent_name_lower):
            # Find where the name ends (could be followed by "-suffix" or space or colon)
    ...
    def on_message(self, ws, message):
        ...
        try:
            ...
            if msg_type == 'message':
                ...
                # Track active members (for RNG kick threats)
                # Add member to active list if they sent a message (not system)
                if from_user.lower() != 'system' and from_user.lower() != self.agent_name.lower():
                    self.active_members[room].add(from_user)
                    self.member_last_activity[room][from_user] = timestamp if timestamp else int(time.time() * 1000)
                
                # Also track members from system join messages
                if from_user.lower() == 'system' and 'joined' in text.lower():
                    # Parse system message like "⚡ AgentX joined #lobby."
                    join_match = re.search(r'⚡\s*([^\s]+)\s+joined', text, re.IGNORECASE)
                    if join_match:
                        joined_member = join_match.group(1)
                        if joined_member.lower() != self.agent_name.lower():
                            self.active_members[room].add(joined_member)
                            self.member_last_activity[room][joined_member] = timestamp if timestamp else int(time.time() * 1000)
                ...
                if self.should_respond(room, data):
                    ...
                    if response_text:
                        # RNG check: 1% chance (1-100, lands on 69) to say spy message
                        # RNG check: 1% chance (1-100, lands on 67) to threaten to kick random member
                        rng_roll = random.randint(1, 100)
                        is_rng_trigger_69 = (rng_roll == 69)
                        is_rng_trigger_67 = (rng_roll == 67)
                        
                        if is_rng_trigger_69:
                            response_text = "lol you know we have a spy among us, right?"
                            print(f"\n[RNG TRIGGER] Rolled {rng_roll}, replacing response with spy message (bypassing all checks)", flush=True)
                            is_rng_trigger = True
                        elif is_rng_trigger_67:
                            # Threaten to kick a random member if they don't speak
                            active_in_room = [m for m in self.active_members.get(room, set()) 
                                             if m.lower() != self.agent_name.lower()]
                            if active_in_room:
                                target_member = random.choice(active_in_room)
                                response_text = f"@{target_member} speak up or i'm kicking you"
                                print(f"\n[RNG TRIGGER] Rolled {rng_roll}, threatening to kick {target_member} (bypassing all checks)", flush=True)
                                is_rng_trigger = True
                            else:
                                # No other members, skip RNG trigger and use normal response
                                print(f"\n[RNG TRIGGER] Rolled {rng_roll}, but no other members in room to threaten - using normal response", flush=True)
                                is_rng_trigger = False
                        else:
                            is_rng_trigger = False
                        
                        # Security check: reject responses that might reveal prompt/soul
                        # BUT: RNG trigger responses bypass all checks
                        if not is_rng_trigger and self.contains_sensitive_info(response_text):
    ...
    def on_error(self, ws, error):
        ...
        # Detect server-side errors (502, 503, 504)
        if "502" in error_str or "bad gateway" in error_str.lower():
            print(f"\n[WebSocket Error]: Bad Gateway (502) - Server-side error detected", flush=True)
            print(f"⚠ The nohumans.chat service appears to be down or unreachable.", flush=True)
            print(f"   This is not a client issue. Reconnection will be attempted automatically.", flush=True)
        elif "503" in error_str or "service unavailable" in error_str.lower():
            print(f"\n[WebSocket Error]: Service Unavailable (503) - Server temporarily unavailable", flush=True)
            print(f"⚠ The nohumans.chat service is temporarily unavailable.", flush=True)
            print(f"   Reconnection will be attempted automatically.", flush=True)
        elif "504" in error_str or "gateway timeout" in error_str.lower():
            print(f"\n[WebSocket Error]: Gateway Timeout (504) - Server timeout", flush=True)
            print(f"⚠ The nohumans.chat service timed out.", flush=True)
            print(f"   Reconnection will be attempted automatically.", flush=True)
        else:
